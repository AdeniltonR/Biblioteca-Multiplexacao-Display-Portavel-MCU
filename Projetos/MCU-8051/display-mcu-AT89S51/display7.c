 ...
 * PROJETO: Display7.c
 * VERSAO: 01
 * DESCRICAO: - feat: Configuracao do MCU.
...
void display_exibir_numero(int numero) {
    ...
    centenas = (numero / 100) % 10;                                    // Obtem o digito das centenas
    dezenas  = (numero / 10) % 10;                                     // Obtem o digito das dezenas
    unidades = (numero / 1) % 10;                                      // Obtem o digito das unidades
...
void display_exibir_decimal(float numero) {
    ...
    parte_inteira =  (int)numero;                                              // Extrai a parte inteira
    casas_decimais = (int)((numero - parte_inteira) * 100 + 0.5);              // Extrai duas casas decimais

    dezenas  = (parte_inteira / 10) % 10;                              // Obtem o digito das dezenas (se houver)
    unidades = (parte_inteira % 10);                                   // Obtem o digito das unidades

    primeira_decimal = casas_decimais / 10;                            // Primeira casa decimal
    segunda_decimal  = casas_decimais % 10;                            // Segunda casa decimal
